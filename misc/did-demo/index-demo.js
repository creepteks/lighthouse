...
app.get('/', (req, res) => {
  credentials.createDisclosureRequest({
    ...
    res.send(`<h3>Scan the QRCode to login to service using your DID</h3><div><img src="${qr}"/></div>`)
  })
})
...
app.get('/verifyId', (req, res) => {
  credentials.createDisclosureRequest({
    ...
    const uri = message.paramsToQueryString(message.messageToURI(requestToken), { callback_type: 'post' })
    const qr = transports.ui.getImageDataURI(uri)
    res.send(`<h3>Scan the QRCode to prove to service that you own the VC you have received</h3><div><img src="${qr}"/></div>`)
  })
})
